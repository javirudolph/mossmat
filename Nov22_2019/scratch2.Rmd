...
There are several NA values, probably from missing measurements of replicates dying.
...
If we average these values across technical replicates (clones), then we can get values for most of of the traits for most individuals.
```{r}
trait_raw[,c(1,2,5,17:19)] %>%
  group_by(sampid) %>% # sampid is the unique identifier for the clones
  summarise_at(vars(starts_with("Leaf")), mean, na.rm = TRUE) %>% 
  mutate_at(vars(starts_with("Leaf")), list(~ as.numeric(ifelse(. == "NaN", "NA", .)))) -> leaf_data_av

# Some NAs
leaf_data_av[!complete.cases(leaf_data_av), ]
...
Also, unlike the leaf data, these traits have already been averaged by individual, since the technical replicates have the same values for each individual: 
```{r}
head(
...
#  nrow()
  pivot_longer(., cols = starts_with("Avg"),
               names_to = "trait", values_to =  "value") -> grodev_data
  
grodev_data %>% 
  ggplot(aes(x = trait, y = value)) +
  geom_boxplot() +
  labs(y = "Trait values", x = "Traits") +
...
# Change those outliers
grodev_data[which(grodev_data$value > 100),]$value <- grodev_data[which(grodev_data$value > 100),]$value / 1000

# Find the negative numbers
grodev_data[which(grodev_data$value < 0),]$value <- grodev_data[which(grodev_data$value < 0),]$value /-10
...
trait_raw[,c(1,2,5,10:16)] %>% 
  ...
  # filter(duplicated(sampid) | duplicated(sampid, fromLast = TRUE))
  # group_by(famid,sampid, sex) %>% 
  # summarise_at(vars(starts_with("Avg")), mean, na.rm = TRUE) %>% 
  # mutate_at(vars(starts_with("Avg")), list(~ as.numeric(ifelse(. == "NaN", "NA", .)))) %>% 
  # ungroup() -> test
```
