...
These side by side correlations are donw for all of them, then by sex. The first panel will show the correlations organized by mass, whereas the second panel will use a clustering algorithm.
```{r}
# Correlation
...
We can tell there are some compounds that can be grouped.
```{r}
voc_corr <- cor(voc_ordination_data[,4:78])
corrplot(voc_corr, method = "circle", order = "hclust", tl.col = "black", tl.cex = .5, addrect = 16)
```
