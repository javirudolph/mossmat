...
master_less$ID <- seq.int(nrow(master_less))
str(master_less)
...
master_less %>% 
  ...
  full_join(., reprovar) %>% 
  distinct() %>%
  set_names(c("Family", "Sample", "Sex", "raw_repro", "repro","leaf_length", "leaf_area",
              "leaf_perim")) -> L_R_Traits
```
...
reprovar %>% 
  select(-c(famid, sampid, Avg_Male_Buds.Stem, Avg_Arch)) %>% 
  gather(key = "trait", value = "raw_values") %>% 
  ggplot(aes(x = raw_values, y = ..density..)) +
  geom_histogram(na.rm = TRUE, fill = "#8e9998") +
  facet_wrap( ~ trait, scales = "free")
